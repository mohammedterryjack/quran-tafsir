                    ...
                    Align(
                      ...
                      child: InkWell(
                        ...
                        onTap: () async {
                          ...
                          _model.apiResulto4c = await GetQuranCall.call(
                            chapter: _model.chapterNo,
                            verse: _model.verseNo,
                          );
                          _shouldSetState = true;
                          if ((_model.apiResulto4c?.succeeded ?? true)) {
                            setState(() {
                              _model.jsonResponse =
                                  (_model.apiResulto4c?.jsonBody ?? '');
                            });
                          } else {
                            if (_shouldSetState) setState(() {});
                            return;
                        ...
                        child: RichText(
                          text: TextSpan(
                            children: [
                              ...
                              TextSpan(
                                text: (List<String> verses, List<double> scores,
                                        List<String> chapterNames) {
                                  return "\n" +
                                      verses
                                          .asMap()
                                          .map((index, verse) => MapEntry(index,
                                              '${(scores[index] * 100).toInt()}% :  [${verse.split(':')[1]} - ${chapterNames[int.parse(verse.split(':')[0]) - 1]} ]'))
                                          .values
                                          .join('\n ');
                                }(
                                    _model.similarVerses.toList(),
                                    _model.similarityScores.toList(),
                                    _model.chapterNames.toList()),
                                style: TextStyle(
                                  color: FlutterFlowTheme.of(context).success,
                                  fontWeight: FontWeight.w100,
